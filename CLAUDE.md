...
| `estadistica` 🟡 | tablas-simples (2 ✅ + ficha-3 pendiente, sin registrar en materiales.html) · pictogramas | ~6 |
...
1. **1º Primaria Matemáticas** — estructura acordada con Miguel el 2026-05-24 (detalle completo en sección "Estructura matemáticas Primaria"). 6 bloques (5 LOMLOE + problemas), sub-bloques anidados, **6 fichas/serie**, curso a curso 1º→6º. **Total 1º Mates: 120 fichas**.
   ...
   - **Bloque `estadistica` EN MARCHA 🟡 — `tablas-simples`** (2/3 fichas, **falta ficha-3 + registrar las 3 en `materiales.html`**). Sin SVG nuevos en ficha-1 (reutiliza figuras planas); en ficha-2 se introducen SVGs B&N de frutas (manzana, plátano, pera, uva — reusables). Tablas con `<table class="tabla-rellenar">` HTML real (border-collapse, th gris). Patrón validado: contar mosaico→tabla, leer tabla→preguntas (MÁS/MENOS/TOTAL), ordenar de MENOS a MÁS, dibujar tantas como dice la tabla.
   ...
   - **Siguiente:** cerrar **ficha-3 de `tablas-simples`** (repaso con escena de aula/animales: lápices/gomas/libros u objetos del aula, tabla con preguntas, V/F sobre tabla, ordenar de mayor a menor). Luego registrar las 3 en `materiales.html` y arrancar **`pictogramas`** (gráfico de barras con iconos: cada 🍎 = 1).
