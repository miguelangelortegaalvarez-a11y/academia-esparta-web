...
| `medida` ⏳ EN MARCHA | longitud-km-m-cm-mm ✅ · masa-kilo-gramo · capacidad-litro-mililitro · el-reloj-y-los-minutos · el-dinero-euros-y-centimos · superficie |
...
**Bloque `medida` de 3º — EN MARCHA:**
- **`longitud-km-m-cm-mm` COMPLETO ✅ (6 fichas, cerrado 2026-06-22, commit a9b1d39) — abre el bloque `medida` de 3º.** Nivel acordado con Miguel (AskUserQuestion): **solo km, m, cm, mm** (sin la escalera hm/dam/dm) y **conversiones enteras sin decimales** (decimales→4º). f1 introducción (4 unidades + equivalencias 1km=1000m / 1m=100cm / 1cm=10mm / 1m=1000mm + elegir unidad + completar equivalencias + ordenar mayor→menor + medir con regla en cm + medida lógica) · f2 convertir (km→m ×1000, m→cm ×100, cm→mm ×10, m→mm ×1000 + al revés) · f3 comparar y ordenar (signo >,<,= pasando a misma unidad + rodear la mayor + ordenar menor→mayor + V/F) · f4 sumar y restar longitudes (misma unidad) + 2 problemas · f5 6 problemas de longitud (rejilla 2×3, componente `.problema`) · f6 repaso. f1 a mano (ancla) + f2-f6 con **5 agentes en paralelo (1/ficha), números fijados y verificados por mí**. Reusa la **regla SVG de 2º** (33px=1cm, objeto rect desde x=24) y el `.problema` de operaciones 3º. Validador `--virtual-time-budget=4000`: 1 folio A4 c/u. **SIGUIENTE sub-bloque: `masa-kilo-gramo`** (reusar balanza SVG de 2º). Resto del árbol: capacidad-litro-mililitro · el-reloj-y-los-minutos · el-dinero-euros-y-centimos · superficie. Reusables de 2º: regla SVG (33px=1cm), balanza, vaso con nivel, reloj analógico, SVG dinero.
...
- **3º Primaria Matemáticas — EN MARCHA** (arrancado 2026-06-15, fuente Poppins):
  ...
  - **Bloque `medida` EN MARCHA**: `longitud-km-m-cm-mm` (6) ✅ (commit a9b1d39, cerrado 2026-06-22 — solo km/m/cm/mm, conversiones enteras sin decimales; f1 a mano + f2-f6 con 5 agentes/1 ficha; reusa regla SVG de 2º + `.problema`) — **siguiente sub-bloque: `masa-kilo-gramo`**
